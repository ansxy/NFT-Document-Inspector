...
export default function NavBarNew() {
  ...
  useEffect(() => {
    const checkAddressWallet = async () => {
      ...
      setIsConnected(JSON.parse(window.localStorage.getItem("connected")));
    };
    checkAddressWallet();
  }, []);
        ...
        <ul className="md:flex hidden uppercase items-center gap-44 px-5 rounded-b-2xl drop-shadow-lg bg-navbar">
          ...
          {isConnected ? (
            ...
            <button
              ...
              className="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-full text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2"
            >
              <svg
                aria-hidden="true"
